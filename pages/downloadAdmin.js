    ...
    const generatePDF = async () => {
        ...
        img.onload = () => {
            ...
            for (let i = 1; i <= requiredPages; i++) {
                ...
                doc.setFontSize(8);
                doc.text("Universität zu Köln • Albertus-Magnus-Platz • 50923 Köln", 20, 30);
                ...
                for (let resIndex = dataPerPage * (i - 1); resIndex < dataPerPage * (i - 1) + dataPerPage; resIndex++) {
                    if (resIndex < responseMessage.length) {
                        ...
                        doc.line(5, distanceToTop + (resIndex % dataPerPage) * distanceLines- 5, 205, distanceToTop + (resIndex % dataPerPage) * distanceLines-5);
                        doc.setFontSize(10);
                        doc.text(responseMessage[resIndex].block_name, 5, distanceToTop + (resIndex % dataPerPage) * distanceLines);
                        doc.setFontSize(12);
                        doc.text(responseMessage[resIndex].semester, 70, distanceToTop + (resIndex % dataPerPage) * distanceLines);
                        doc.text(responseMessage[resIndex].matrikelnummer, 110, distanceToTop + (resIndex % dataPerPage) * distanceLines);
                        doc.text("" + parseInt(responseMessage[resIndex].percentage, 10) + "%", 150, distanceToTop + (resIndex % dataPerPage) * distanceLines);
                    }
                }
